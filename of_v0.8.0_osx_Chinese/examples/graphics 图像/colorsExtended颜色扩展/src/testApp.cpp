...
void testApp::setup(){
    ...
    // 这个对照表在我们需要用字符串检索颜色的时候非常有用。为了便于分类这些颜色，我们把他们放在一个矢量里面。
    ...
    sortedType = 1; // by name, at the start 通过名字检索颜色。
...
void testApp::draw(){
    ...
    //画所有的颜色
    //需要注意的是，这些代码其实可以被优化，因为我们有很多颜色是画在屏幕之外的。
    ...
    //按1按照名字分类，按2按照色调分类，按3按照明度分类，按4按照饱和度分类
